...
public:
  Tester() : mrs_uav_testing::TestGeneric() {
    sch_start_ = mrs_lib::ServiceClientHandler<mrs_msgs::srv::Vec1>(node_, "/uav1/sweeping_generator/start");
  }

  bool test(void);

private:
  mrs_lib::ServiceClientHandler<mrs_msgs::srv::Vec1> sch_start_;
};
...
bool Tester::test(void) {
  ...
  while (true) {
    ...
    RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000, "[%s]: waiting till not having a goal", name_.c_str());
