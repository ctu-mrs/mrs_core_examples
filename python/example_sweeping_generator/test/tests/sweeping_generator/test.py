...
def generate_test_description():
    ...
    this_pkg="example_sweeping_generator"
    ...
    # custom_config=get_package_share_directory(this_pkg)+"/config/example_custom_config.yaml"
    ...
    ld.add_action(
        GroupAction([
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource([
                    PathJoinSubstitution([
                        FindPackageShare('example_sweeping_generator'),
                            'launch',
                            'sweeping_generator.launch.py'
                        ])
                    ])
                )
            ]
...
class PublisherHandlerTest(unittest.TestCase):
    ...
    def test_interactor(self, proc_output, timeout=250):
