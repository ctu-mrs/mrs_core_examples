            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource([
                    PathJoinSubstitution([
                        FindPackageShare('example_waypoint_flier_native'),
                            'launch',
                            'waypoint_flier_native.launch.py'
                        ])
                    ])
                )
            ]
...
class Test(unittest.TestCase):
    ...
    def test_logs_crashing(self, proc_output):
        proc_output.assertWaitFor('waypoint flier is not activated yet', timeout=3.0, stream='stderr')
